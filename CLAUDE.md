...
Requires `cargo-zigbuild`. Worker SSH hosts are hardcoded in `deploy/deploy.sh` (`ivantha-worker-0`, `ivantha-worker-1`). The agent runs as a systemd **user** service (`~/.config/systemd/user/watchman-agent.service`).
